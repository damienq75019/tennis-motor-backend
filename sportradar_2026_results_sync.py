...
from postgres_results_store import PostgresResultsStore, row_unique_key
...
class Results2026Syncer:
    ...
    def status(self) -> Dict[str, Any]:
        store = PostgresResultsStore()
        db_status = store.status() if store.enabled else {
        ...
        return {
            "status": "ok",
            "provider": "sportradar",
            "target": "PostgreSQL + data/2026.csv rebuild" if store.enabled else "data/2026.csv",
            "storageMode": "postgres" if store.enabled else "csv",
            "csvPath": str(self.csv_path),
            "csvExists": self.csv_path.exists(),
            "csvRows": _count_csv_rows(self.csv_path),
            "postgres": db_status,
    ...
    def postgres_status(self) -> Dict[str, Any]:
        store = PostgresResultsStore()
        return {
            "status": "ok" if store.enabled else "not_configured",
            "storageMode": "postgres" if store.enabled else "csv",
            "postgres": store.status() if store.enabled else {
                "databaseConfigured": False,
                "databaseStatus": "not_configured",
                "error": "DATABASE_URL absente dans le service web.",
            },
    ...
    def export_postgres_to_csv(self) -> Dict[str, Any]:
        ...
        try:
            store.ensure_schema()
            result = store.export_csv(self.csv_path)
            result.update({
                "status": "ok",
                "storageMode": "postgres",
                "postgresRows": store.count_rows(),
                "csvRows": _count_csv_rows(self.csv_path),
            })
            return result
    ...
    def sync_day(self, target_day: str, *, dry_run: bool = False) -> Dict[str, Any]:
        ...
        store = PostgresResultsStore()
        use_postgres = store.enabled
        audit["storage"] = {
            "mode": "postgres" if use_postgres else "csv",
            "databaseConfigured": use_postgres,
            "dbSeedImport": None,
            "dbRowsBefore": None,
            "dbRowsAfter": None,
            "csvRebuild": None,
        ...
        db_existing_keys: Optional[Set[str]] = None
        if use_postgres:
            try:
                # En mode PostgreSQL, on refuse de retomber silencieusement sur un CSV éphémère.
                store.ensure_schema()
                if not dry_run:
                    seed = store.import_csv_if_empty(self.csv_path)
                    audit["storage"]["dbSeedImport"] = seed
                audit["storage"]["dbRowsBefore"] = store.count_rows()
                db_existing_keys = store.existing_keys()
            except Exception as exc:
                audit["status"] = "error"
                audit["errors"].append(f"postgres unavailable: {type(exc).__name__}: {exc}")
                return audit
        ...
        existing = _existing_keys(self.csv_path) if not use_postgres else set()
        ...
        for idx, summary in enumerate(atp_summaries, start=1):
            ...
            winner_history_name = _display_name_for_history(winner_comp["name"])
            loser_history_name = _display_name_for_history(loser_comp["name"])

            key = (
                tourney_date,
                _canon(tourney_name),
                _canon(winner_history_name),
                _canon(loser_history_name),
                score,
            )
            semantic_key = row_unique_key({
                "tourney_date": tourney_date,
                "tourney_name": tourney_name,
                "winner_name": winner_history_name,
                "loser_name": loser_history_name,
                "score": score,
            })
            if use_postgres:
                if db_existing_keys is not None and semantic_key in db_existing_keys:
                    audit["counts"]["skipped_duplicate"] += 1
                    continue
                if db_existing_keys is not None:
                    db_existing_keys.add(semantic_key)
            else:
                if key in existing:
                    audit["counts"]["skipped_duplicate"] += 1
                    continue
            ...
            # Extra metadata used only by PostgreSQL storage. csv.DictWriter ignores it.
            row["sport_event_id"] = event_id
            rows_to_add.append(row)
            if len(audit["addedSample"]) < 10:
                audit["addedSample"].append({
                    "eventId": event_id,
                    "tourney": tourney_name,
                    "winner": winner_history_name,
                    "loser": loser_history_name,
                    "winnerSourceName": winner_comp["name"],
                    "loserSourceName": loser_comp["name"],
                    "score": score,
                    "round": round_code,
                })
        ...
        if rows_to_add and not dry_run:
            if use_postgres:
                inserted, conflict_skipped = store.insert_rows(rows_to_add, source="sportradar")
                if conflict_skipped:
                    audit["counts"]["skipped_duplicate"] += conflict_skipped
                audit["counts"]["rows_added"] = inserted
                audit["storage"]["dbRowsAfter"] = store.count_rows()
                audit["storage"]["csvRebuild"] = store.export_csv(self.csv_path)
            else:
            ...
            audit["counts"]["rows_added"] = 0
            if use_postgres and not dry_run:
                audit["storage"]["dbRowsAfter"] = store.count_rows()
