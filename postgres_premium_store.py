...
class PostgresPremiumStore:
    ...
    def void_row_by_id(
        ...
        *,
        ...
        Used by API-Tennis legacy-name fallback when old Sportradar rows
    ...
    def repair_shelton_merida_20260525(self) -> Dict[str, Any]:
        ...
        STEP40 could not resolve this legacy Sportradar row through API-Tennis
        ...
        with self._connect() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    f"""
                    SELECT id, date, status, category, predicted_winner, opponent, result, real_winner, score
                    FROM {self.TABLE}
                    WHERE id = %s
                    """,
                    (row_id,),
                ...
                before = {
                    ...
                    "category": _s(before_row[3]),
                    "predictedWinner": _s(before_row[4]),
                    "opponent": _s(before_row[5]),
                    "result": _s(before_row[6]),
                    "realWinner": _s(before_row[7]),
                    "score": _s(before_row[8]),
                }
                ...
                after = {
                    ...
                    "category": _s(after_row[3]),
                    "predictedWinner": _s(after_row[4]),
                    "opponent": _s(after_row[5]),
                    "result": _s(after_row[6]),
                    "realWinner": _s(after_row[7]),
                    "score": _s(after_row[8]),
                } if after_row else {}
            conn.commit()
        return {
            "status": "ok",
    ...
    def repair_wawrinka_fils_dejong_20260525(self) -> Dict[str, Any]:
        ...
        with self._connect() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    f"""
                    SELECT id, date, status, category, source_player_a, source_player_b,
                ...
                before = {
                    ...
                    "status": _s(before_row[2]),
                    "category": _s(before_row[3]),
                    "sourcePlayerA": _s(before_row[4]),
                    "sourcePlayerB": _s(before_row[5]),
                    "predictedWinner": _s(before_row[6]),
                    "opponent": _s(before_row[7]),
                    "result": _s(before_row[8]),
                    "realWinner": _s(before_row[9]),
                    "score": _s(before_row[10]),
                    "oddPredicted": _s(before_row[11]),
                    "oddOpponent": _s(before_row[12]),
                }
                ...
                after = {
                    ...
                    "status": _s(after_row[2]),
                    "category": _s(after_row[3]),
                    "sourcePlayerA": _s(after_row[4]),
                    "sourcePlayerB": _s(after_row[5]),
                    "predictedWinner": _s(after_row[6]),
                    "opponent": _s(after_row[7]),
                    "result": _s(after_row[8]),
                    "realWinner": _s(after_row[9]),
                    "score": _s(after_row[10]),
                    "oddPredicted": _s(after_row[11]),
                    "oddOpponent": _s(after_row[12]),
                } if after_row else {}
            conn.commit()
