...
def verified_recovery_rows() -> List[Dict[str, Any]]:
    """
    Reset-compatible: plus aucune ligne premium codée en dur.

    Ancienne version : restaurait automatiquement Alexander Zverev / Alexander Blockx
    quand premium_history.json était vide après reset ou redéploiement.

    Nouvelle règle : si l'utilisateur reset l'historique Premium, il doit rester à zéro.
    """
    return []
...
def restore_verified_if_history_empty() -> Dict[str, Any]:
    """
    Reset-compatible : ne restaure plus jamais de premium codé en dur.
    """
    return {"restoredVerifiedRows": 0, "reason": "recovery_disabled"}
