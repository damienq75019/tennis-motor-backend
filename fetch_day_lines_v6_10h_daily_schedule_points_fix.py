...
# -*- coding: utf-8 -*-
"""
Tennis Motor - Fetch daily lines V6.10H DAILY SCHEDULE POINTS FIX
...
Utilisation :
    py fetch_day_lines_v6_10h_daily_schedule_points_fix.py today --backend-url http://127.0.0.1:8000
    py fetch_day_lines_v6_10h_daily_schedule_points_fix.py tomorrow --backend-url http://127.0.0.1:8000
    py fetch_day_lines_v6_10h_daily_schedule_points_fix.py today --backend-url http://127.0.0.1:9
...
MODE = "V6_10H_DAILY_SCHEDULE_POINTS_FIX"
...
def _canonical_no_accents(name: str) -> str:
    return canonical_name(_strip_accents_basic(name or ""))


def build_points_alias_map(points_map: Dict[str, int], display_map: Optional[Dict[str, str]] = None) -> Dict[str, int]:
    aliases: Dict[str, int] = {}

    def add(alias: str, points: int) -> None:
        try:
            p = int(points or 0)
        except Exception:
            p = 0
        if p <= 0:
            return
        for key in {canonical_name(alias), _canonical_no_accents(alias)}:
            if key and key not in aliases:
                aliases[key] = p

    for key, points in (points_map or {}).items():
        add(str(key), int(points or 0))
    ...
    for full in (display_map or {}).values():
        ...
        p = 0
        for key in {canonical_name(full_name), _canonical_no_accents(full_name)}:
            if key in aliases:
                p = aliases[key]
                break
            if key in points_map:
                try:
                    p = int(points_map[key] or 0)
                except Exception:
                    p = 0
                break
        ...
        if len(parts) >= 2:
            ...
            add(f"{parts[0][0]}. {surname}", p)
            add(f"{parts[0][0]} {surname}", p)
            add(f"{parts[0][0]}. {parts[-1]}", p)
            add(f"{parts[0][0]} {parts[-1]}", p)

    manual_groups = [
        ["Federico Cina", "Federico Cinà", "F Cina", "F. Cina"],
        ["Alexander Blockx", "A Blockx", "A. Blockx"],
        ["Pablo Carreno Busta", "Pablo Carreno", "Pablo Carreno-Busta", "Carreno Busta", "P Carreno Busta", "P. Carreno Busta"],
        ["Alejandro Tabilo", "A Tabilo", "A. Tabilo"],
        ["Zhizhen Zhang", "Zhi Zhen Zhang", "Z Zhang", "Z. Zhang"],
        ["Nuno Borges", "N Borges", "N. Borges"],
        ["Jesper de Jong", "Jesper De Jong", "J de Jong", "J. de Jong", "J De Jong", "J. De Jong"],
        ["Botic van de Zandschulp", "Botic Van De Zandschulp", "B van de Zandschulp", "B. van de Zandschulp"],
        ["Alexandre Muller", "Alexandre Müller", "A Muller", "A. Muller"],
        ["Jan-Lennard Struff", "Jan Lennard Struff", "J Struff", "J. Struff"],
        ["Francisco Comesana", "Francisco Comesaña", "F Comesana", "F. Comesana"],
        ["Jacob Fearnley", "J Fearnley", "J. Fearnley"],
        ["Giovanni Mpetshi Perricard", "G Mpetshi Perricard", "G. Mpetshi Perricard"],
    ]

    for group in manual_groups:
        p = 0
        for name in group:
            p = aliases.get(canonical_name(name)) or aliases.get(_canonical_no_accents(name)) or 0
            if p:
                break
        if p:
            for name in group:
...
def _points_for_name_with_aliases(name: str, points_map: Dict[str, int], aliases: Dict[str, int]) -> int:
    clean = clean_name(name)
    keys = [canonical_name(clean), _canonical_no_accents(clean)]

    parts = clean.split()
    if len(parts) >= 2:
        surname = " ".join(parts[1:])
        keys.extend([
            canonical_name(f"{parts[0][0]}. {surname}"),
            canonical_name(f"{parts[0][0]} {surname}"),
            canonical_name(f"{parts[0][0]}. {parts[-1]}"),
            canonical_name(f"{parts[0][0]} {parts[-1]}"),
        ])

    for key in keys:
        if key in aliases:
            return int(aliases[key] or 0)
        if key in points_map:
            try:
                return int(points_map[key] or 0)
            except Exception:
                pass
...
def build_payload_items_direct_from_schedule_rows(
    ...
    points_alias_map = build_points_alias_map(points_map, globals().get("_DISPLAY_MAP_FOR_POINTS_ALIASES", {}))
    audit.append(f"points_alias_map_size={len(points_alias_map)}")
    ...
    for row in schedule_rows:
        ...
        points_a = _points_for_name_with_aliases(player_a, points_map, points_alias_map)
        points_b = _points_for_name_with_aliases(player_b, points_map, points_alias_map)
...
def main() -> int:
    ...
    audit.append("source_policy=ATP_DAILY_SCHEDULE_POINTS_FIX_ONLY")
    ...
    globals()["_DISPLAY_MAP_FOR_POINTS_ALIASES"] = display_map
