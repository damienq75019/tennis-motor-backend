...
def _name_variants(value: Any) -> List[str]:
    ...
    if "," in raw:
        ...
        if len(parts) >= 2:
            # Sportradar often stores "Last, First" while API-Tennis uses "First Last".
...
class PremiumHistorySyncer:
    ...
    def settle_day_from_api_tennis(self, target_day: str, *, dry_run: bool = False, builder: Optional[ApiTennisDailyBuilder] = None) -> Dict[str, Any]:
        ...
        return {
            ...
            "policy": "Règlement STEP39 API-Tennis + fallback legacy Sportradar : pending -> win/loss via winner_id API-Tennis; retired/walkover/cancelled/abandoned -> void/remboursé.",
        }
