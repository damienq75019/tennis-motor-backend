...
# -*- coding: utf-8 -*-
"""
Tennis Motor - Fetch daily lines V6.10E DAILY SCHEDULE SCROLL WAIT
...
Utilisation :
    py fetch_day_lines_v6_10e_daily_schedule_scroll_wait.py today --backend-url http://127.0.0.1:8000
    py fetch_day_lines_v6_10e_daily_schedule_scroll_wait.py tomorrow --backend-url http://127.0.0.1:8000
    py fetch_day_lines_v6_10e_daily_schedule_scroll_wait.py today --backend-url http://127.0.0.1:9
...
MODE = "V6_10E_DAILY_SCHEDULE_SCROLL_WAIT"
...
def discover_daily_schedule_urls(session, include_challenger: bool = False) -> List[str]:
    ...
    for url in urls_to_scan:
        html = fetch_html_with_scroll(session, url, audit=None, purpose="discover")
...
# ---------------------------------------------------------------------------
# Chargement Playwright complet : attente + scroll
# ---------------------------------------------------------------------------


def fetch_html_with_scroll(session, url: str, audit: Optional[List[str]] = None, purpose: str = "daily_schedule") -> str:
    """
    V6.10E : charge la page ATP avec Playwright, attend le JS,
    scrolle lentement pour déclencher le lazy-loading, puis récupère le HTML final.

    Si Playwright échoue, fallback propre vers base.fetch_html.
    """
    ...
    try:
        ...
        add_audit(f"[SCROLL FETCH FALLBACK] Playwright import impossible | {purpose} | {url} | {exc}")
        return base.fetch_html(session, url)
    ...
    try:
        with sync_playwright() as p:
            ...
            # Tentative légère de fermer une éventuelle bannière cookies si elle existe.
            for label in ["Accept All", "Accept all", "I Accept", "Agree", "OK"]:
                try:
            ...
            # Scroll progressif : déclenche les contenus chargés seulement à l'affichage.
            scroll_steps = 18
            previous_height = 0
            stable_count = 0
            ...
            for i in range(scroll_steps):
                try:
                    current_height = int(page.evaluate("document.body.scrollHeight || document.documentElement.scrollHeight || 0"))
                except Exception:
                    current_height = 0
                ...
                try:
                    new_height = int(page.evaluate("document.body.scrollHeight || document.documentElement.scrollHeight || 0"))
                except Exception:
                ...
                if stable_count >= 4 and i >= 6:
                    break

            # Petite pause finale, puis retour haut pour stabiliser le DOM.
            ...
            player_link_count = len(re.findall(r"/players/", html or "", flags=re.I))
            add_audit(
                f"[SCROLL FETCH OK] {purpose} | url={url} | html_len={len(html or '')} | "
                f"player_links={player_link_count} | scroll_steps={scroll_steps}"
            )
        ...
        add_audit(f"[SCROLL FETCH FALLBACK] {purpose} | {url} | {exc}")
        try:
...
def build_daily_schedule_matches(session, target_day: date, include_challenger: bool) -> Tuple[List[Any], List[str], List[Dict[str, str]], Dict[str, Optional[str]]]:
    ...
    for url in schedule_urls:
        ...
        try:
            html = fetch_html_with_scroll(session, url, audit=audit, purpose="daily_schedule")
...
# ---------------------------------------------------------------------------
# V6.10E - Construction payload directe depuis daily-schedule
# ---------------------------------------------------------------------------
...
def main() -> int:
    ...
    audit.append("source_policy=ATP_DAILY_SCHEDULE_SCROLL_WAIT_ONLY")
