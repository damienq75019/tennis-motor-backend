...
def _api_tennis_name_aliases(name: str) -> List[str]:
    """
    Génère des alias sûrs pour les noms API-Tennis mal ordonnés.

    Problème vu le 26/05 : API-Tennis renvoie parfois les prénoms / noms
    espagnols et latino-américains dans un ordre différent de Flashscore.

    Exemples API-Tennis -> Flashscore attendu :
    - "Andres Burruchaga Roman" -> "Burruchaga R." / "Burruchaga R. A."
    - "Carlos Prado Angelo Juan" -> "Prado Angelo J. C."
    - "Manuel Cerundolo Juan" -> "Cerundolo J. M."
    - "Daniel Vallejo Adolfo" -> "Vallejo D." ou "Vallejo A. D."

    On ne touche pas au moteur. Cela sert uniquement au matching des cotes.
    """
    raw = (name or "").strip()
    aliases: List[str] = []
    ...
    # Si le nom est déjà au format "Nom, Prénom", le profil standard suffit.
    if "," in raw:
        return aliases

    norm_tokens = _flashscore_tokens_keep_initials(raw)
    display_tokens = re.findall(r"[A-Za-zÀ-ÿ]+", raw)

    if len(norm_tokens) != len(display_tokens) or len(norm_tokens) < 3:
        return aliases

    lower = [t.lower() for t in norm_tokens]
    disp = display_tokens

    # Cas générique très fréquent : Prenom Nom Prenom2
    # Exemple : Andres Burruchaga Roman -> Burruchaga, Roman Andres
    # On ajoute aussi l'ordre inverse pour couvrir Flashscore si l'initiale utilisée est celle du premier prénom.
    if len(disp) == 3:
        first, surname, trailing = disp[0], disp[1], disp[2]
        add(f"{surname}, {trailing} {first}")
        add(f"{surname}, {first} {trailing}")

    # Cas : Carlos Prado Angelo Juan -> Prado Angelo, Juan Carlos
    # Nom composé sur 2 tokens au milieu.
    if len(disp) == 4:
        first = disp[0]
        surname_2 = f"{disp[1]} {disp[2]}"
        trailing = disp[3]
        add(f"{surname_2}, {trailing} {first}")
        add(f"{surname_2}, {first} {trailing}")

        # Variante prudente si Flashscore tronque le nom composé.
        add(f"{disp[2]}, {trailing} {first} {disp[1]}")

    # Cas ciblés validés dans Tennis Motor.
    joined = " ".join(lower)
    targeted = {
        "andres burruchaga roman": ["Burruchaga, Roman Andres", "Burruchaga, Andres Roman"],
        "carlos prado angelo juan": ["Prado Angelo, Juan Carlos", "Prado Angelo, Carlos Juan"],
        "manuel cerundolo juan": ["Cerundolo, Juan Manuel", "Cerundolo, Manuel Juan"],
        "daniel vallejo adolfo": ["Vallejo, Daniel Adolfo", "Vallejo, Adolfo Daniel"],
    }
    for alias in targeted.get(joined, []):
        add(alias)
...
def _full_name_profile(name: str) -> Dict[str, Any]:
    """
    Profil robuste pour comparer API-Tennis / Sportradar ↔ Flashscore.
...
def _full_name_profiles(name: str) -> List[Dict[str, Any]]:
    profiles: List[Dict[str, Any]] = []
    seen = set()

    for alias in _api_tennis_name_aliases(name):
        key = _norm_name(alias)
        if not key or key in seen:
            continue
        seen.add(key)
        profiles.append(_full_name_profile(alias))
...
def _same_player_flashscore(full_name: str, flash_name: str) -> bool:
    """
    Matching robuste ATP/API-Tennis full name <-> Flashscore.

    STEP45 : essaie aussi les alias API-Tennis pour corriger les cotes manquantes
    sur les noms comme Burruchaga Roman, Cerundolo Juan, Prado Angelo Juan,
    Vallejo Adolfo.
    """
    ...
    for full in _full_name_profiles(full_name):
        if not full.get("ordered_tokens"):
            continue
        ...
        if flash_surname and flash_initials:
            ...
            # dans le nom de famille et les initiales correspondent.
...
def _flashscore_match_keys(name: str) -> List[str]:
    """
    Clés très tolérantes pour matcher ATP/API-Tennis ↔ Flashscore.
    STEP45 : inclut aussi les alias de noms API-Tennis mal ordonnés.
    """
    keys: List[str] = []

    def add_key(key: str) -> None:
        key = (key or "").strip()
        if key and len(key) >= 3 and key not in keys:
            keys.append(key)
    ...
    for alias in _api_tennis_name_aliases(name):
        ...
        # Si alias au format "Nom, Prénom", _name_tokens enlève la virgule.
        # On ajoute les candidats de nom de famille calculés par profil.
        for profile in _full_name_profiles(alias):
            for surname in profile.get("surname_candidates") or []:
                add_key(str(surname))
...
def enrich_result_with_flashscore_odds(result: Dict[str, Any], target_day: str) -> Dict[str, Any]:
    ...
    for match in matches:
        ...
        if found:
            ...
            if len(unmatched_sample) < 12:
                unmatched_sample.append(f"{player_a} - {player_b}")
    ...
    result["daily"]["flashscoreAudit"] = (flash_audit + " | matched_sample=" + " || ".join(matched_sample) + " | unmatched_sample=" + " || ".join(unmatched_sample))[-5000:]
