...
def row_unique_key(row: Dict[str, Any]) -> str:
    ...
    We intentionally do not depend only on provider event_id because older CSV rows do not have it.
    """
    parts = [
...
class PostgresResultsStore:
    ...
    def insert_rows(self, rows: Iterable[Dict[str, Any]], *, source: str = "api_tennis") -> Tuple[int, int]:
        self.ensure_schema()
        inserted = 0
        skipped = 0
        with self._connect() as conn:
