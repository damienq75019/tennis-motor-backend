...
# -*- coding: utf-8 -*-
"""
Tennis Motor - Fetch daily lines V6.10G DAILY SCHEDULE POINTS ALIASES
...
Utilisation :
    py fetch_day_lines_v6_10g_daily_schedule_points_aliases.py today --backend-url http://127.0.0.1:8000
    py fetch_day_lines_v6_10g_daily_schedule_points_aliases.py tomorrow --backend-url http://127.0.0.1:8000
    py fetch_day_lines_v6_10g_daily_schedule_points_aliases.py today --backend-url http://127.0.0.1:9
...
MODE = "V6_10G_DAILY_SCHEDULE_POINTS_ALIASES"
...
def build_points_alias_map(points_map: Dict[str, int], display_map: Optional[Dict[str, str]] = None) -> Dict[str, int]:
    """
    V6.10G :
    Construit des alias robustes pour les points ATP.
    Corrige les cas classiques :
    - accents : Muller / Müller, Munar / Munar
    - particules : van de Zandschulp
    - noms composés : Carreno Busta
    - initiale + nom : F. Cina, J. Fearnley
    """
    ...
    # 1) Tous les points_map existants.
    for key, points in (points_map or {}).items():
        add(str(key), int(points or 0))
        add(_strip_accents_basic(str(key)), int(points or 0))

    # 2) Noms complets du display_map.
    for full in (display_map or {}).values():
        full_name = clean_name(str(full or ""))
        if not full_name:
            continue
        ...
        p = 0
        for k in {canonical_name(full_name), _canonical_no_accents(full_name)}:
            if k in points_map:
                try:
                    p = int(points_map[k] or 0)
                except Exception:
                    p = 0
                break
            if k in aliases:
        ...
        if len(parts) >= 2:
            ...
            add(f"{first[0]}. {surname}", p)
            add(f"{first[0]} {surname}", p)
            add(f"{first[0]}. {parts[-1]}", p)
            add(f"{first[0]} {parts[-1]}", p)

    # 3) Alias manuels sûrs de normalisation ATP.
    manual_aliases = {
        ...
        "Francisco Comesaña": "Francisco Comesana",
        "Jacob Fearnley": "Jacob Fearnley",
        "Giovanni Mpetshi Perricard": "Giovanni Mpetshi Perricard",
        "Federico Cina": "Federico Cina",
        "Federico Cinà": "Federico Cina",
        "Alejandro Tabilo": "Alejandro Tabilo",
        "Zhizhen Zhang": "Zhizhen Zhang",
        "Zhi Zhen Zhang": "Zhizhen Zhang",
    }

    # Si une des deux formes existe déjà avec des points, propager à l'autre.
    for a, b in manual_aliases.items():
        pa = aliases.get(canonical_name(a)) or aliases.get(_canonical_no_accents(a)) or 0
        pb = aliases.get(canonical_name(b)) or aliases.get(_canonical_no_accents(b)) or 0
        p = int(pa or pb or 0)
        if p > 0:
            add(a, p)
            add(b, p)
...
def _points_for_name_with_aliases(name: str, points_map: Dict[str, int], points_alias_map: Dict[str, int]) -> int:
    keys = [
        canonical_name(name),
        _canonical_no_accents(name),
    ]
    ...
    if len(parts) >= 2:
        surname = " ".join(parts[1:])
        keys.append(canonical_name(f"{parts[0][0]}. {surname}"))
        keys.append(canonical_name(f"{parts[0][0]} {surname}"))
        keys.append(canonical_name(f"{parts[0][0]}. {parts[-1]}"))
        keys.append(canonical_name(f"{parts[0][0]} {parts[-1]}"))
    ...
    for key in keys:
        ...
        if key in points_alias_map:
            return int(points_alias_map.get(key) or 0)

        if key in points_map:
            try:
                return int(points_map.get(key) or 0)
            except Exception:
                pass
...
def build_payload_items_direct_from_schedule_rows(
    ...
    points_alias_map = build_points_alias_map(points_map, display_map or {})
...
def main() -> int:
    ...
    audit.append("source_policy=ATP_DAILY_SCHEDULE_POINTS_ALIASES_ONLY")
