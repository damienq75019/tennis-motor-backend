...
app = FastAPI(title="Tennis Motor Backend Clean", version="step60-step56-official-engine")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
def _apply_step56_official_fields(chosen: Dict[str, Any], audit: Dict[str, Any], step49_best: Dict[str, Any], original_score: float, reversed_score: float) -> Dict[str, Any]:
    """STEP60: make STEP56 the official probability/decision while preserving STEP49 traces."""
    ...
    chosen["decision"] = "✅ Jouable" if confidence > 0.80 and not veto else "❌ Pas jouable"
    chosen["officialEngine"] = "STEP56 Global Direct Prediction"
    chosen["officialEngineVersion"] = "step60-step56-official-engine"
...
def calculate_from_matches(matches: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for match in matches:
        ...
        if isinstance(audit, dict) and audit.get("status") == "ok":
            ...
            chosen["officialEngineVersion"] = "step60-step56-official-engine-fallback"
    ...
    return {
        ...
        "engine": {
            ...
            "serviceVersion": "step60-step56-official-engine",
            "historyYears": list(HISTORY_YEARS),
        ...
        "daily": {
            ...
            "contextPropagation": "step60_step56_official_with_step49_trace_round_wins_veto_active",
            "excludedPlayers": _excluded_analysis_names(),
...
def root() -> Dict[str, Any]:
    return {
        ...
        "version": "step60-step56-official-engine",
        "coreEngineVersion": "step56-global-direct-prediction-official",
        "message": "Backend STEP59 : STEP49 officiel inchangé + STEP56 audit-only dark endpoint. Aucun appel Sportradar en production.",
        "endpoints": ["/health", "/calculate", "/predictions", "/state", "/history", "/daily", "/api-tennis/status", "/odds/status", "/sync/results2026/status", "/sync/results2026/run", "/sync/results2026/postgres/status", "/sync/results2026/postgres/export", "/sync/premium/status", "/sync/premium/list", "/sync/premium/reset", "/sync/premium/run", "/sync/premium/settle", "/sync/premium/settle-pending", "/sync/history/form-value", "/sync/history/list", "/sync/history/reset", "/sync/history/repair-dellien-royer", "/sync/history/repair-shelton-merida", "/sync/history/repair-wawrinka-fils-dejong", "/sync/history/repair-van-assche-kypson-gaubas", "/sync/history/settle", "/sync/history/settle-pending", "/sync/daily-maintenance/run", "/audit/step56/status", "/audit/step56/daily", "/audit/step56/calculate"],
...
def health() -> Dict[str, Any]:
    ...
    return {
        ...
        "version": "step60-step56-official-engine",
        "coreEngineVersion": "step56-global-direct-prediction-official",
        "step56Official": "enabled_official_engine",
        ...
        "qualifierDetectorActivation": "tournament_wins_engine_veto_active_qualifier_audit_only",
        "clayVetoPolicy": "active_on_clay_if_opponent_round_wins_before_match_ge_2_unless_extreme_structural_advantage",
        ...
        "step56Policy": "official_daily_engine_no_odds_no_player_names_as_features_history_write_enabled",
    }
...
def daily(day: str = Query("today"), auto_history: bool = Query(True), provider: str = Query("api_tennis")) -> Dict[str, Any]:
    ...
    # STEP60 : aucune cote dans le calcul officiel.
