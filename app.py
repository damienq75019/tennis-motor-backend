...
app = FastAPI(title="Tennis Motor Backend Clean", version="step42b-wawrinka-repair-schema-fix")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
def root() -> Dict[str, Any]:
    return {
        ...
        "endpoints": ["/health", "/calculate", "/predictions", "/state", "/history", "/daily", "/api-tennis/status", "/sportradar/status", "/odds/status", "/sync/results2026/status", "/sync/results2026/run", "/sync/results2026/postgres/status", "/sync/results2026/postgres/export", "/sync/premium/status", "/sync/premium/list", "/sync/premium/reset", "/sync/premium/run", "/sync/premium/settle", "/sync/premium/settle-pending", "/sync/history/form-value", "/sync/history/list", "/sync/history/reset", "/sync/history/repair-dellien-royer", "/sync/history/repair-shelton-merida", "/sync/history/repair-wawrinka-fils-dejong", "/sync/history/settle", "/sync/history/settle-pending"],
...
def odds_status() -> Dict[str, Any]:
    ...
    return {
        ...
        "serviceVersion": "step42b-wawrinka-repair-schema-fix",
    }
...
def sync_results2026_status() -> Dict[str, Any]:
    ...
    status["serviceVersion"] = "step42b-wawrinka-repair-schema-fix"
    return status
...
def sync_results2026_postgres_export() -> Dict[str, Any]:
    ...
    result["serviceVersion"] = "step42b-wawrinka-repair-schema-fix"
...
def sync_history_form_value(
    ...
    try:
        ...
        report["serviceVersion"] = "step42b-wawrinka-repair-schema-fix"
        return report
    except Exception as exc:
        return {
            "status": "error",
