...
app = FastAPI(title="Tennis Motor Backend Clean", version="step46-daily-api-tennis-only")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
def calculate_from_matches(matches: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    return {
        ...
        "daily": {
            ...
            "contextPropagation": "clean_step46-daily-api-tennis-only_11_real_history_signals_placeholder_filter_qualifier_audit_only",
            "excludedPlayers": _excluded_analysis_names(),
...
def _apply_form_value_engine(response: Dict[str, Any]) -> None:
    ...
    if not isinstance(matches, list) or not matches:
        response["daily"]["formValueEngine"] = {
            "status": "skipped",
            "reason": "no_matches",
            "version": "step46-daily-api-tennis-only",
        }
        return
    ...
    for match in matches:
        ...
        match.update(value)
        match["formValueEngineVersion"] = "step46-daily-api-tennis-only"
...
def root() -> Dict[str, Any]:
    return {
        ...
        "version": "step46-daily-api-tennis-only",
        "message": "Backend STEP46 : API-Tennis uniquement. Aucun appel Sportradar en production.",
...
def daily(day: str = Query("today"), auto_history: bool = Query(True), provider: str = Query("api_tennis")) -> Dict[str, Any]:
    ...
    # STEP46 : API-Tennis est le fournisseur unique.
...
def odds_status() -> Dict[str, Any]:
    ...
    return {
        ...
        "serviceVersion": "step46-daily-api-tennis-only",
    }
...
def sportradar_status() -> Dict[str, Any]:
    return {
        ...
        "sportradarDisabled": True,
        "apiTennisOnly": True,
        "message": "STEP46 : Sportradar est désactivé. Utilise /api-tennis/status et /daily avec API-Tennis.",
        "serviceVersion": "step46-daily-api-tennis-only",
    }
...
def sync_results2026_status() -> Dict[str, Any]:
    ...
    status["serviceVersion"] = "step46-daily-api-tennis-only"
    return status
...
def sync_results2026_postgres_export() -> Dict[str, Any]:
    ...
    result["serviceVersion"] = "step46-daily-api-tennis-only"
...
def sync_history_form_value(
    ...
    try:
        ...
        report["serviceVersion"] = "step46-daily-api-tennis-only"
        return report
    except Exception as exc:
        return {
            "status": "error",
...
def sync_daily_maintenance_run(
    ...
    """STEP46 : route quotidienne unique API-Tennis.
    ...
    try:
        if daily_payload.get("status") == "ok":
            history_sync = syncer.sync_daily_result(daily_payload, target_day, dry_run=dry_run)
        else:
            history_sync = {"status": "skipped", "reason": "daily_payload_not_ok"}
