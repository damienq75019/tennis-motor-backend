...
app = FastAPI(title="Tennis Motor Backend Clean", version="step43-van-assche-kypson-gaubas-void")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    ...
    for match in matches or []:
        if _is_placeholder_match(match):
            ...
            removed.append({
                ...
                "message": "Joueurs pas encore connus par Sportradar.",
...
def calculate_from_matches(matches: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    if not matches and not not_analyzed_matches:
        if placeholder_removed and not excluded_removed:
            ...
            message = "Aucun match exploitable : Sportradar ne donne pas encore les vrais joueurs."
        ...
        response["daily"]["placeholderFilterPolicy"] = "hide_sportradar_tbd_wsf_winner_placeholders"
    ...
    return {
        ...
        "daily": {
            ...
            "contextPropagation": "clean_step2_11_real_history_signals_placeholder_filter_qualifier_audit_only",
            "excludedPlayers": _excluded_analysis_names(),
...
def root() -> Dict[str, Any]:
    return {
        ...
        "endpoints": ["/health", "/calculate", "/predictions", "/state", "/history", "/daily", "/api-tennis/status", "/sportradar/status", "/odds/status", "/sync/results2026/status", "/sync/results2026/run", "/sync/results2026/postgres/status", "/sync/results2026/postgres/export", "/sync/premium/status", "/sync/premium/list", "/sync/premium/reset", "/sync/premium/run", "/sync/premium/settle", "/sync/premium/settle-pending", "/sync/history/form-value", "/sync/history/list", "/sync/history/reset", "/sync/history/repair-dellien-royer", "/sync/history/repair-shelton-merida", "/sync/history/repair-wawrinka-fils-dejong", "/sync/history/repair-van-assche-kypson-gaubas", "/sync/history/settle", "/sync/history/settle-pending"],
...
def health() -> Dict[str, Any]:
    ...
    return {
        ...
        "placeholderFilter": "hide_sportradar_tbd_wsf_winner_placeholders",
...
def odds_status() -> Dict[str, Any]:
    ...
    return {
        ...
        "serviceVersion": "step43-van-assche-kypson-gaubas-void",
    }
...
def sync_results2026_status() -> Dict[str, Any]:
    ...
    status["serviceVersion"] = "step43-van-assche-kypson-gaubas-void"
    return status
...
def sync_results2026_postgres_export() -> Dict[str, Any]:
    ...
    result["serviceVersion"] = "step43-van-assche-kypson-gaubas-void"
...
def sync_history_form_value(
    ...
    try:
        ...
        report["serviceVersion"] = "step43-van-assche-kypson-gaubas-void"
        return report
    except Exception as exc:
        return {
            "status": "error",
...
def sync_history_reconcile_all(
    ...
    - Corrige les anciens IDs Sportradar par fallback noms joueurs.
